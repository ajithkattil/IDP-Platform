...
Once `idp-platform-ai` is deployed and the pipeline is running,
...
## Step 1 — Deploy idp-platform-ai

Run the pipeline once to deploy the service:

```bash
cd services/idp-platform-ai
git push origin main
# Pipeline: lint → sast → test → docker → ECR → EKS deploy
# Service URL after deploy: http://idp-platform-ai.internal.idp.com
```
...
| `GITLAB_PROJECT_IDS_JSON` | `{"idp-platform-ai":"12345","spring-orders-poc":"12346"}` | Get project IDs from GitLab project settings |
...
Open `idp_devportal_v3_complete.html` in a browser.

In the POC screen, click the **"Connect to live POC"** button (top right).
Enter the service URL: `http://idp-platform-ai.internal.idp.com`
...
# 3. Wait for pods
kubectl wait deployment/idp-platform-ai --for=condition=available -n platform-ai
...
eksctl delete cluster --name idp-poc-demo --region us-east-1
```
...
- Look at service logs: `kubectl logs -n platform-ai -l app=idp-platform-ai`
