/**
 * Zayo IDP Portal — Production Grade v2
 ...
 *   localhost:8000  → zayo-platform-ai (Claude Sonnet 4)
...
const ICONS = {
  ...
  zayoLogo:  <svg viewBox="0 0 20 20" fill="none"><path d="M3 10L10 3L17 10L10 17Z" stroke="white" strokeWidth="1.8" fill="none"/><circle cx="10" cy="10" r="3" fill="white"/></svg>,
      ...
      <div style={{padding:"16px 14px 14px", borderBottom:`1px solid ${DS.b1}`}}>
        <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:13}}>
          <div style={{width:28,height:28,borderRadius:8,background:DS.ac,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:`0 0 0 1px ${DS.acGlow}, ${DS.sh1}`}}>
            <Icon id="zayoLogo" size={16}/>
          </div>
          <div>
            <div style={{fontSize:13.5,fontWeight:700,color:DS.t1,letterSpacing:"-.03em",lineHeight:1.2}}>Zayo DevPortal</div>
...
function Dashboard({ live, onNav }) {
  ...
  const pipelines = [
    {name:"zayo-platform-ai", stages:[1,1,1,1,1,1,1], status:"green", time:"4m 12s"},
  ...
  const activity = [
    {c:DS.ok, label:"Deploy",  text:<><strong style={{color:DS.t1}}>zayo-platform-ai</strong> v1.0.0-909fea02 deployed · ArgoCD sync · 1/1 healthy</>,   time:"2m ago"},
...
function Catalog({ live }) {
  ...
  const DEMO = [
    {name:"zayo-platform-ai", type:"C",typc:"blue",  system:"developer-platform",  owner:"platform-engineering", lang:"Python 3.11", target:"EKS",       source:"GitLab",     s:[1,1,1,1,1,1,1], st:"green",  domain:"Platform"},
...
function Copilot({ live }) {
  const [msgs, setMsgs] = useState([{role:"ai",text:"I have context on your POC services — zayo-platform-ai and spring-orders-poc are live on EKS. Backstage catalog is running headless. What would you like to know?"}]);
  ...
  const send = async (text) => {
    ...
    try {
      const res = await fetch(`${AI_URL}/api/v1/chat`,{
        ...
        body:JSON.stringify({message:q,context:"Zayo IDP POC on EKS us-east-1. zayo-platform-ai (Python/FastAPI), spring-orders-poc (Java Spring Boot), Backstage headless catalog. All running via ArgoCD."})
      });
  ...
  return (
    <div className="page-enter" style={{display:"flex",flexDirection:"column",height:"calc(100vh - 90px)"}}>
      <div className="stagger-1" style={{marginBottom:14,flexShrink:0}}>
        ...
        <div style={{fontSize:12.5,color:DS.t3}}>Claude Sonnet 4 · powered by zayo-platform-ai · live on EKS us-east-1</div>
      </div>
      ...
      <div className="stagger-2" style={{flex:1,display:"grid",gridTemplateColumns:"1fr 260px",gap:12,minHeight:0}}>
        ...
        <div style={{background:DS.bg3,border:`1px solid ${DS.b2}`,borderRadius:DS.r10,display:"flex",flexDirection:"column",overflow:"hidden",boxShadow:DS.sh1}}>
          ...
          <div style={{padding:"12px 16px",borderBottom:`1px solid ${DS.b1}`,display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
            ...
            <div>
              <div style={{fontSize:13,fontWeight:700,color:DS.t1,letterSpacing:"-.02em"}}>Zayo AI Copilot</div>
      ...
      <div className="stagger-3">
        <Card>
          ...
          <div style={{padding:"12px 16px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {[
              {ns:"platform-ai",pods:[{n:"zayo-platform-ai",st:"ok",r:"1/1",rs:0}]},
      ...
      <div className="stagger-4">
        <Card>
          ...
          <div style={{padding:"12px 16px",display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
            {[
              ["GitLab repo",`gitlab.com/zayo-group/devops/idp-platform/${svc}`],
              ["ECR image",`501149494381.dkr.ecr.us-east-1.amazonaws.com/zayo-poc/${svc}`],
  ...
  return (
    <div className="page-enter" style={{display:"flex",flexDirection:"column",gap:14}}>
      <div className="stagger-1">
        ...
        <div style={{fontSize:12.5,color:DS.t3}}>AWS us-east-1 · account 501149494381 · POC environment</div>
      </div>
      <div className="stagger-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        {[
          ...
          {t:"AWS infrastructure",sub:"Terraform managed",rows:[["S3 state","zayo-poc-tf-state-501149494381-idp"],["DynamoDB","zayo-poc-tf-locks"],["OIDC provider","gitlab.com (exists)"],["Subnets","subnet-0d32dc7e · subnet-0ce39a84"]]},
          {t:"GitLab CI/CD",sub:"Zayo GitLab group",rows:[["Group","zayo-group/devops/idp-platform"],["Pipelines","3 repos · all green"],["Runners","GitLab SaaS · Linux · small"],["Group vars","12 variables"]]},
        ].map((c,i)=>(
          <Card key={i}>
            <div style={{padding:"12px 14px",borderBottom:`1px solid ${DS.b1}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div>
