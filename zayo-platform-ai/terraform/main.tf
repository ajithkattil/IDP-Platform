# ============================================================
# Terraform — Zayo POC EKS Cluster + Supporting Infrastructure
...
#   - ECR repository for zayo-platform-ai
...
variable "cluster_name"  { type = string
  default = "zayo-poc-eks"
}
variable "environment"   { type = string
  default = "prod"
}
variable "service_name"  { type = string
  default = "zayo-platform-ai"
}
...
module "eks" {
  ...
  access_entries = {
    platform-ci = {
      kubernetes_groups = []
      principal_arn     = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/zayo-poc-gitlab-ci-role"
      policy_associations = {
        admin = {
...
resource "aws_ecr_repository" "service" {
  name                 = "zayo-poc/${var.service_name}"
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:secretsmanager:${var.aws_region}:${data.aws_caller_identity.current.account_id}:secret:zayo/${var.service_name}/*"
      },
      {
        Effect   = "Allow"
