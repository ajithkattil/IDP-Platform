...
**Port:** 8000  
**Image:** 501149494381.dkr.ecr.us-east-1.amazonaws.com/zayo-poc/zayo-platform-ai  
**GitLab:** gitlab.com/cltajith/zayo-platform-ai

---

## What it does

The AI brain of the Zayo IDP. It runs Claude Sonnet 4 (Anthropic) and provides:
...
This creates:
- S3 bucket for Terraform state: `zayo-poc-tf-state-501149494381-idp`
- DynamoDB lock table: `zayo-poc-tf-locks`
- ECR repos: `zayo-poc/zayo-platform-ai` and `zayo-poc/spring-orders-poc`
- GitLab OIDC provider (may be blocked by permission boundary — use static credentials instead)

### 2. Set GitLab CI/CD variables

Go to `gitlab.com/cltajith/zayo-platform-ai` → Settings → CI/CD → Variables:
...
```bash
git clone https://gitlab.com/cltajith/zayo-platform-ai.git
cd zayo-platform-ai
git add .
...
kubectl create secret generic zayo-platform-ai-secrets \
  -n platform-ai \
...
# View logs
kubectl logs -n platform-ai -l app=zayo-platform-ai --tail=50

# Port forward
kubectl port-forward svc/zayo-platform-ai 8000:8000 -n platform-ai

# Restart deployment
kubectl rollout restart deployment/zayo-platform-ai -n platform-ai

# Force ArgoCD sync
kubectl annotate application zayo-platform-ai \
  -n argocd \
  "argocd.argoproj.io/refresh=hard" --overwrite

# Recreate secret (after credential rotation)
kubectl delete secret zayo-platform-ai-secrets -n platform-ai
kubectl create secret generic zayo-platform-ai-secrets \
  -n platform-ai \
  --from-literal=anthropic-api-key="sk-ant-..." \
  --from-literal=database-url="" \
